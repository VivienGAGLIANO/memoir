...
Pour comprendre l'intérêt du signal analytique, l'exemple d'une fonction sinusoïdale est présentée, d'amplitude $A$ et de fréquence $\omega_0$ fixées
\begin{equation}
    f(t) = A\sin(\omega_0t).
\end{equation}
...
Le problème à l'analyse de ce signal est le fait qu'il existe plusieurs niveaux de structure, comme expliqué dans la section~\ref{subsec:structure}. Les différents niveaux de structure interfèrent et rendent l'interprétation des informations locales impossible à l'échelle macroscopique. Pour extraire de l'information interprétable, il faut sélectionner et faire l'analyse d'un seul niveau de structure, donc d'échelle. Un filtrage est fait pour sélectionner un niveau de structure en particulier et enlever les composantes des niveaux non désirés. 
...
\end{equation}
Cette approximation donne ici de bons résultats, car les filtres ont une réponse équivalente à celle de la transformée de Riesz aux alentours de $\frac\pi2$, où se concentre la majorité de l'énergie des images dans la pyramide laplacienne~\cite{wadhwa_riesz_2014}. Plusieurs travaux~\cite{torralba_statistics_2003, zoran_daniel_natural_2013} relèvent en effet que les images naturelles, images de scènes et paysages communs dans la nature, ont une distribution d'énergie fortement concentrée autour de l'origine dans le domaine de Fourier, et que cette énergie diminue proportionnellement à $1/f^2$ en s'éloignant de l'origine, avec $f$ la fréquence. En $0$, il y a une différence notable entre la réponse des filtres et la réponse de la transformée de Riesz. Cependant, le contenu fréquentiel est faible en $0$ puisque, par construction, le contenu basse fréquences est supprimé dans la pyramide laplacienne. L'erreur due à la différence en 0 est donc faible. Il ne reste ainsi que les fréquences intermédiaires, où l'approximation a une réponse équivalente à la transformée exacte. Un cas d'application où de la prudence et des vérifications supplémentaires seraient nécessaires est celui des textures comportant une quantité importante de contenu haute fréquence. De telles textures pourraient contenir des fréquences mal approximées par les filtres présentés, et quand même préservées dans la pyramide laplacienne. L'aspect visuel de telles textures serait cependant différent de celui des textures ici considérées, et d'autres méthodes de synthèse seraient sûrement plus adaptées pour traiter ces cas, notamment des méthodes de synthèse par bruit procédural comme le texton noise~\cite{galerne_texton_2017}. Wadhwa \textit{et al.}~\cite{wadhwa_riesz_2014} relèvent aussi dans leur contenu supplémentaire que l'approximation ne préserve pas toujours exactement l'amplitude du signal, ce qui peut impacter légèrement la phase. Pour palier à cela, ils recommandent le lissage spatial de la phase, qui permet de réduire l'impact de cette inexactitude. Ce lissage n'est pas mis en place dans ce travail, mais serait une piste intéressante pour l'amélioration de la qualité de l'approximation. Une explication plus détaillée, ainsi que la justification de la valeur des filtres utilisés, est faite dans le contenu supplémentaire de Wadhwa \textit{et al.} disponible sur \href{https://people.csail.mit.edu/nwadhwa/riesz-pyramid/}{cette page}. La figure~\ref{fig:riesz-approximation} montre une comparaison entre l'application de la transformée de Riesz et de son approximation.

\bigskip

Une fois la transformée de Riesz de l'image calculée, les informations locales pour chaque étage, amplitude, phase et orientation, sont ensuite dérivées comme expliqué dans la section précédente. La figure~\ref{fig:riesz-pyramid-local} présente les informations locales obtenues avec la pyramide de Riesz. Avec cette implémentation de la pyramide de Riesz, le cadre d'étude multirésolution local a été défini. La prochaine partie décrit comment le cadre multirésolution local est utilisé afin d'extraire de l'information d'une texture.
...
Une nouvelle expression de la congruence de phases qui prend en compte la localisation fréquentielle
\begin{equation}
