...
avec $M^e_n$ et $M^o_n$ les filtres passe-bande, paire et impaire, au niveau d'échelle $n$. {\color{red}COMMENT FORMULER ÇA?} À noter que le $n$ dans l'équation~\ref{eq:local-scale-amplitude} désigne le niveau d'échelle fréquentiel, et n'est pas le même que celui dans l'équation de la série de Fourier~\ref{eq:phase-congruence-energy} qui, lui, représente les différentes sinusoïdes qui forment le signal. En regardant le scalogramme représenté à la figure~\ref{fig:phase-congruence-scalogram} qui représente l'analyse du signal par la banque de filtres, on voit l'intérêt de la congruence de phase : les points saillants sont ceux où la phase est alignée à travers tous les niveaux d'échelle.

\begin{figure}
    \centering
    \includegraphics[width=.75\textwidth]{contenu/resources/images/phase_congruence_scalogram}
...
\begin{equation}
    PC(x) = \frac{E(x)}{\sum_{n=0}^{+\infty} A_n(x)} = \frac{\sqrt{F^2(x)+F_H^2(x)}}{\epsilon + \sum_{n=0}^{+\infty}},
\end{equation}
...
\begin{align}
    F(x) &= \sum_{n=0}^{+\infty} f(x)*M_n^e, \,\text{et}\\
    F_H(x) &= \sum_{n=0}^{+\infty} f(x)*M{_n^o},
\end{align}

et $\sum_{n=0}^{+\infty} A_n(x)$ la somme des amplitudes locales à tous les niveaux d'échelle :

\begin{equation}
    \sum_{n=0}^{+\infty} A_n(x) = \sum_{n=0}^{+\infty} \sqrt{(f(x)*M^e_n)^2 + (f(x)*M^o_n)^2}
\end{equation}
...
\begin{figure}
           \centering
           \includegraphics[width=.65\textwidth]{contenu/resources/images/wavelet_spectrum_coverage}
...
Pour étendre la méthode en deux dimensions, Kovesi utilise une banque de filtres de Gabor 2D auxquels il applique une fonction d'étalement, perpendiculairement à la direction du filtre. En variant l'orientation des filtres, il s'assure de recouvrir totalement le spectre de l'image. L'expression de la congruence de phase s'étend pour prendre en compte toutes les orientations :

\begin{equation}
    PC(\mathbf{x}) = \frac{\sum_{o\in \mathcal{O}} E_o(\mathbf{x})}{\epsilon + \sum_{o \in \mathcal{O}}\sum_{n=0}^{+\infty} A_{no}(\mathbf{x})}
\end{equation},

où $\mathcal{O}$ est l'ensemble des orientations utilisées dans la banque de filtres.
